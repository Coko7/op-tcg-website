...
const Card: React.FC<CardProps> = ({
  ...
  return (
    ...
    >
      {isNew && (
        <div className="absolute -top-2 -right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full font-bold animate-bounce-in z-10">
          NOUVEAU!
        </div>
      )}

      {quantity !== undefined && (
        <div className="absolute top-3 left-3 bg-black/80 text-white text-sm px-3 py-1 rounded-full font-bold z-10">
          x{quantity}
        </div>
      )}
      ...
      <div className="p-4 bg-gradient-to-br from-gray-800 to-gray-900">
        ...
        <div className="text-center mb-3">
          <h3 className="font-bold text-white text-lg leading-tight mb-1">{card.name}</h3>
          <p className="text-white/80 text-sm">{card.character}</p>
          ...
          <div className="flex items-center justify-center space-x-2 text-xs mt-2">
            ...
            {card.type && (
              <span className="bg-gray-700 text-white/90 px-2 py-1 rounded">{card.type}</span>
            )}
          </div>
          ...
          <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                {card.cost !== undefined && (
                  <div className="bg-gray-700/50 rounded p-2">
                    ...
                    <div className="text-white font-bold text-lg">{card.cost}</div>
                  </div>
                )}
                {card.power !== undefined && (
                  <div className="bg-gray-700/50 rounded p-2">
                    <div className="text-white/60 text-xs font-medium">POWER</div>
                    <div className="text-white font-bold text-lg">{card.power}</div>
                  </div>
                )}
                {card.counter !== undefined && (
                  <div className="bg-gray-700/50 rounded p-2">
                    <div className="text-white/60 text-xs font-medium">COUNTER</div>
                    <div className="text-white font-bold text-lg">{card.counter}</div>
                  </div>
                )}
              </div>
            )}
            ...
            {!(card.cost !== undefined || card.power !== undefined || card.counter !== undefined) && (
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center justify-center space-x-2 bg-gray-700/50 rounded p-2">
                  <Sword size={16} className="text-red-400" />
                  <span className="text-white font-bold">{card.power || card.attack}</span>
                </div>
                <div className="flex items-center justify-center space-x-2 bg-gray-700/50 rounded p-2">
                  <Shield size={16} className="text-blue-400" />
                  <span className="text-white font-bold">{card.counter || card.defense}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Description */}
        {card.description && (
          <div className="mt-3 p-3 bg-gray-700/30 rounded text-white/90 text-xs leading-relaxed">
            {card.description}
          </div>
        )}
        ...
        {card.special_ability && (
          <div className="mt-3 p-3 bg-yellow-900/30 border border-yellow-600/30 rounded">
            <div className="text-yellow-400 font-semibold text-xs mb-1">🌟 CAPACITÉ SPÉCIALE</div>
            <div className="text-white/90 text-xs leading-relaxed">{card.special_ability}</div>
          </div>
        )}
      </div>
    </div>
